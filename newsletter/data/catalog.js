...
// email clients load them from absolute URLs; PRODUCT LINKS use the REAL live-store handles
// (verified against hooknloop.com.au/products.json) so every "Shop" link resolves.
...
const BROWSE = `${SHOP_BASE}/collections/all` // real applications pages don't exist on the store
...
// The store has no per-industry pages, so cards link to the browse-all collection.
...
function ind(file) { return `${ASSET_BASE}/img/industries/${file}` }
