import { ASSET_BASE, SHOP_BASE } from '../data/catalog.js'
...
const button = (label, href, { solid = true } = {}) => `
  <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin:0;"><tr>
    <td align="center" style="border-radius:10px;${solid ? 'background:#e8590c;' : 'border:1.5px solid #0b2447;'}">
      <a href="${href}" style="display:inline-block;padding:${solid ? '13px 26px' : '12px 24px'};font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;line-height:1;letter-spacing:.2px;text-decoration:none;color:${solid ? '#ffffff' : '#0b2447'};">${label}</a>
    </td>
  </tr></table>`
  ...
  <tr><td style="padding:8px 32px 0;">
    <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #e9e4db;border-radius:16px;overflow:hidden;background:#ffffff;">
      ...
      <tr><td style="padding:22px 24px 24px;">
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase;color:#e8590c;">This week&rsquo;s pick</div>
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:20px;font-weight:700;line-height:1.3;color:#0b2447;margin:7px 0 3px;letter-spacing:-.2px;">${p.title}</div>
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#6a7180;">${p.category} &middot; from <span style="color:#101a2b;font-weight:700;">$${p.price}</span></div>
...
const arrivalCard = (p) => !p ? '' : `
  <tr><td style="padding:22px 32px 0;">
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;color:#9aa0aa;margin-bottom:11px;">Fresh in stock</div>
    <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #e9e4db;border-radius:14px;overflow:hidden;background:#ffffff;">
      <tr>
        <td width="160" style="background:#f5f2ec;font-size:0;vertical-align:middle;">
          <img src="${p.image}" alt="${p.title}" width="160" style="display:block;width:160px;height:auto;border:0;">
        </td>
        <td style="padding:16px 20px;vertical-align:middle;">
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:700;line-height:1.3;color:#0b2447;">${p.title}</div>
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#6a7180;margin-top:4px;">from $${p.price}</div>
          <a href="${p.url}" style="font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;color:#e8590c;text-decoration:none;display:inline-block;margin-top:11px;">View product  →</a>
        </td>
      </tr>
    </table>
  </td></tr>`
...
<table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" style="background:#efece6;">
  <tr><td align="center" style="padding:30px 12px;">
    <table role="presentation" width="600" border="0" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 20px 48px -24px rgba(11,36,71,.35);">
      ...
      <tr><td style="background:#0b2447;padding:22px 32px;">
        <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0"><tr>
          ...
          <td align="right" style="vertical-align:middle;font-family:Arial,Helvetica,sans-serif;font-size:10.5px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8ea3c2;">Weekly&nbsp;dispatch</td>
        </tr></table>
      </td></tr>
      ...
      <tr><td style="padding:30px 32px 6px;">
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;color:#e8590c;">${label}</div>
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:25px;font-weight:700;line-height:1.25;color:#0b2447;margin-top:9px;letter-spacing:-.3px;">Fresh from the workshop.</div>
        ${fields.news && fields.news.trim() ? `<p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.65;color:#3a4152;margin:12px 0 0;">${fields.news}</p>` : ''}
      </td></tr>

      ${spotlightCard(spotlight)}
      ${showArrival ? arrivalCard(arrival) : ''}
      ...
      <tr><td style="padding:24px 32px 0;">
        <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" style="background:#0b2447;border-radius:14px;">
          <tr><td style="padding:20px 22px;">
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#f0a06a;">Trade tip</div>
            <p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#dfe6f2;margin:9px 0 0;">${tip}</p>
          </td></tr>
        </table>
      </td></tr>
      ...
      <tr><td style="padding:26px 32px 30px;">
        <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0"><tr>
          <td align="center" style="border-top:1px solid #e9e4db;padding-top:26px;">
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#6a7180;margin-bottom:15px;">Need bulk quantities or a trade account?</div>
      ...
      <tr><td style="background:#0b2447;padding:26px 32px;">
        ...
        <p style="font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.7;color:#9fb0c9;margin:0;">
    ...
    <table role="presentation" width="600" border="0" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;">
      <tr><td style="padding:18px 20px 8px;text-align:center;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.7;color:#9aa0aa;">
  ...
  const text = `${weekLabel(fields.weekOf)} — HooknLoop weekly\n\n` +
    `${fields.news ? fields.news + '\n\n' : ''}` +
    `THIS WEEK'S PICK\n${spotlight ? `${spotlight.title} — from $${spotlight.price}\n${spotlight.url}\n` : ''}\n` +
    `${showArrival ? `FRESH IN STOCK\n${arrival.title} — from $${arrival.price}\n${arrival.url}\n\n` : ''}` +
    `TRADE TIP\n${tip}\n\n` +
